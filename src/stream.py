...
class Dataset:
    ...
    def data_generator(self, data_file: str, batch_size: int):
        """
        Input: data_batch_n, batch_size: ...

        Output: Batches list: [images, labels]
        images: List [batch_size][3072]
    ...
    def sendCIFARBatchFileToSpark(self, tcp_connection, input_batch_file, batch_size, split="train"):
        """
        Input: list of .pkl file: data_batch_1, data_batch_2, ....; batch size.

        Output: payload with batch size then send to the receiver
        """
        if split == "train":
    ...
    def streamCIFARDataset(self, tcp_connection, folder, batch_size):
        """
        Input: folder path, batch size

        Perform sending data to TCP
        """
        CIFAR_BATCHES = [
